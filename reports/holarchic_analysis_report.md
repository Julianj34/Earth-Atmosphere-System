...
**Run:** 2026-08-21T20:46:42.622007+00:00  
**Snapshots:** 283  
...
| H_meso_conv | L2p5 | regional | 0.651 | proxy |
| H_micro_storm | L3 | local | 0.196 | measured |
| H_field_iono | L4 | global | 0.325 | measured |
| H_field_gec | L5 | global | 0.353 | measured |
| H_field_reso | L6 | global | 0.297 | proxy |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=113 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.134 | 113 | negligible |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.155 | 112 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.493 | 283 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.520 | 283 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 248/283 Snapshots (88%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=248, `none`=22, `electric_to_resonance`=10, `micro_to_electric`=3

## 3. Welche Rückkopplung ist plausibel?

- `H_field_iono→H_field_reso`: r=+0.190 (negligible) — Cavity-Hoehe/Leitfaehigkeit moduliert Frequenz und Q
- `H_field_gec→H_field_reso`: r=+0.520 (moderate) — GEC ist die elektrische Architektur der Resonanz
