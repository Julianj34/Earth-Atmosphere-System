...
**Run:** 2026-08-11T09:19:12.594660+00:00  
**Snapshots:** 241  
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=71 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | +0.077 | 71 | negligible |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.238 | 70 | weak |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.510 | 241 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.525 | 241 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 210/241 Snapshots (87%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=210, `none`=22, `electric_to_resonance`=7, `micro_to_electric`=2
