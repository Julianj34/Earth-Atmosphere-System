...
**Run:** 2026-08-03T11:52:17.175882+00:00  
**Snapshots:** 210  
...
| H_meso_conv | L2p5 | regional | 0.540 | proxy |
| H_micro_storm | L3 | local | 0.197 | measured |
| H_field_iono | L4 | global | 0.327 | measured |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=40 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | +0.123 | 40 | negligible |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.241 | 39 | weak |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.529 | 210 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.522 | 210 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 182/210 Snapshots (87%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=182, `none`=22, `electric_to_resonance`=5, `micro_to_electric`=1

## 3. Welche Rückkopplung ist plausibel?

- `H_field_iono→H_field_reso`: r=+0.173 (negligible) — Cavity-Hoehe/Leitfaehigkeit moduliert Frequenz und Q
