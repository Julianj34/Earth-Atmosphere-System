...
**Run:** 2026-09-13T07:32:33.070532+00:00  
**Snapshots:** 370  
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=200 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.251 | 200 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.183 | 199 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.303 | 370 | weak |
| H_field_gec→H_field_reso | field_feedback | +0.504 | 370 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 306/370 Snapshots (83%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=306, `none`=26, `electric_to_resonance`=20, `micro_to_electric`=18

## 3. Welche Rückkopplung ist plausibel?

- `H_field_iono→H_field_reso`: r=+0.240 (weak) — Cavity-Hoehe/Leitfaehigkeit moduliert Frequenz und Q
