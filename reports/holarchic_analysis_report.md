...
**Run:** 2026-09-03T22:41:38.207040+00:00  
**Snapshots:** 333  
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=163 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.214 | 163 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | +0.151 | 163 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.449 | 333 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.513 | 333 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 292/333 Snapshots (88%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=292, `none`=22, `electric_to_resonance`=15, `micro_to_electric`=4
