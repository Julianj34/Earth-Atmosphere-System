...
**Run:** 2026-09-14T07:59:16.062235+00:00  
**Snapshots:** 374  
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=204 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.252 | 204 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.191 | 203 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.298 | 374 | weak |
| H_field_gec→H_field_reso | field_feedback | +0.505 | 374 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 310/374 Snapshots (83%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=310, `none`=26, `electric_to_resonance`=20, `micro_to_electric`=18
