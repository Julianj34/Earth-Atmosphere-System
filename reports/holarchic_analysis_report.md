...
**Run:** 2026-08-21T09:03:52.590629+00:00  
**Snapshots:** 281  
...
| H_meso_conv | L2p5 | regional | 0.652 | proxy |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=111 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.134 | 111 | negligible |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.168 | 110 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.492 | 281 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.522 | 281 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 246/281 Snapshots (88%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=246, `none`=22, `electric_to_resonance`=10, `micro_to_electric`=3

## 3. Welche Rückkopplung ist plausibel?

- `H_field_iono→H_field_reso`: r=+0.196 (negligible) — Cavity-Hoehe/Leitfaehigkeit moduliert Frequenz und Q
- `H_field_gec→H_field_reso`: r=+0.522 (moderate) — GEC ist die elektrische Architektur der Resonanz
