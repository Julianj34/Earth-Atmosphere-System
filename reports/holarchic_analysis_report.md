...
**Run:** 2026-09-02T07:10:46.949124+00:00  
**Snapshots:** 327  
...
|---|---|---|---|---|
| H_external | L0 | global | 0.214 | measured |
...
| H_field_gec | L5 | global | 0.346 | measured |
| H_field_reso | L6 | global | 0.294 | proxy |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=157 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.198 | 157 | negligible |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.159 | 156 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.457 | 327 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.515 | 327 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 287/327 Snapshots (88%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=287, `none`=22, `electric_to_resonance`=14, `micro_to_electric`=4

## 3. Welche Rückkopplung ist plausibel?

- `H_field_iono→H_field_reso`: r=+0.197 (negligible) — Cavity-Hoehe/Leitfaehigkeit moduliert Frequenz und Q
