...
**Run:** 2026-09-18T22:40:28.017620+00:00  
**Snapshots:** 393  
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=223 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.261 | 223 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.196 | 222 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.269 | 393 | weak |
| H_field_gec→H_field_reso | field_feedback | +0.496 | 393 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 326/393 Snapshots (83%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=326, `none`=26, `micro_to_electric`=21, `electric_to_resonance`=20
