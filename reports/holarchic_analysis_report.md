...
**Run:** 2026-09-06T12:13:55.054264+00:00  
**Snapshots:** 343  
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=173 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.225 | 173 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.184 | 172 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.390 | 343 | weak |
| H_field_gec→H_field_reso | field_feedback | +0.511 | 343 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 293/343 Snapshots (85%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=293, `none`=22, `electric_to_resonance`=18, `micro_to_electric`=10
