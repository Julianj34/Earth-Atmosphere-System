...
**Run:** 2026-09-25T18:44:54.086524+00:00  
**Snapshots:** 420  
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=250 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.277 | 250 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.193 | 249 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.242 | 420 | weak |
| H_field_gec→H_field_reso | field_feedback | +0.490 | 420 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 346/420 Snapshots (82%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=346, `none`=27, `micro_to_electric`=25, `electric_to_resonance`=22
...
- `H_field_gec→H_field_reso`: r=+0.490 (moderate) — GEC ist die elektrische Architektur der Resonanz
