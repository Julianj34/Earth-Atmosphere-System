...
**Run:** 2026-08-21T14:59:06.711285+00:00  
**Snapshots:** 282  
...
| H_meso_conv | L2p5 | regional | 0.648 | proxy |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=112 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.143 | 112 | negligible |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.164 | 111 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.492 | 282 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.524 | 282 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 247/282 Snapshots (88%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=247, `none`=22, `electric_to_resonance`=10, `micro_to_electric`=3
...
- `H_field_gec→H_field_reso`: r=+0.524 (moderate) — GEC ist die elektrische Architektur der Resonanz
