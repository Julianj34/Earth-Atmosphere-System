...
**Run:** 2026-09-26T07:50:22.585540+00:00  
**Snapshots:** 422  
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=252 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.277 | 252 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.198 | 251 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.241 | 422 | weak |
| H_field_gec→H_field_reso | field_feedback | +0.490 | 422 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 347/422 Snapshots (82%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=347, `none`=27, `micro_to_electric`=25, `electric_to_resonance`=23
