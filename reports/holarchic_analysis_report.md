...
**Run:** 2026-08-14T15:12:33.484581+00:00  
**Snapshots:** 254  
...
| H_meso_conv | L2p5 | regional | 0.644 | proxy |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=84 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.093 | 84 | negligible |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.197 | 83 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.488 | 254 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.531 | 254 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 219/254 Snapshots (86%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=219, `none`=22, `electric_to_resonance`=10, `micro_to_electric`=3
...
- `H_field_gec→H_field_reso`: r=+0.531 (moderate) — GEC ist die elektrische Architektur der Resonanz
