...
**Run:** 2026-09-23T07:50:14.159211+00:00  
**Snapshots:** 410  
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=240 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.261 | 240 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.201 | 239 | weak |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.261 | 410 | weak |
| H_field_gec→H_field_reso | field_feedback | +0.496 | 410 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 340/410 Snapshots (83%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=340, `none`=27, `micro_to_electric`=22, `electric_to_resonance`=21

## 3. Welche Rückkopplung ist plausibel?

- `H_field_iono→H_field_reso`: r=+0.256 (weak) — Cavity-Hoehe/Leitfaehigkeit moduliert Frequenz und Q
