...
**Run:** 2026-07-29T05:35:15.206413+00:00  
**Snapshots:** 189  
...
| H_meso_conv | L2p5 | regional | 0.335 | proxy |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy` bis echtes OLR zurueckkommt), aber die Historie (n=19) ist noch zu klein fuer eine belastbare Kopplungsschaetzung.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | +0.273 | 19 | weak_low_n |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.020 | 18 | negligible_low_n |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.528 | 189 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.498 | 189 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 161/189 Snapshots (85%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=161, `none`=22, `electric_to_resonance`=5, `micro_to_electric`=1

## 3. Welche Rückkopplung ist plausibel?

- `H_field_iono→H_field_reso`: r=+0.195 (negligible) — Cavity-Hoehe/Leitfaehigkeit moduliert Frequenz und Q
