...
**Run:** 2026-07-25T21:14:21.147102+00:00  
**Snapshots:** 177  
...
| H_meso_conv | L2p5 | regional | 0.072 | proxy |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy` bis echtes OLR zurueckkommt), aber die Historie (n=4) ist noch zu klein fuer eine belastbare Kopplungsschaetzung.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.605 | 4 | strong_low_n |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | +0.279 | 4 | weak_low_n |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.534 | 177 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.497 | 177 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 151/177 Snapshots (85%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=151, `none`=21, `electric_to_resonance`=4, `micro_to_electric`=1
