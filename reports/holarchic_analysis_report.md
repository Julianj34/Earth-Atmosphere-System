...
**Run:** 2026-09-05T11:54:21.481780+00:00  
**Snapshots:** 339  
...
| H_field_gec | L5 | global | 0.345 | measured |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=169 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.218 | 169 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.179 | 168 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.417 | 339 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.511 | 339 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 292/339 Snapshots (86%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=292, `none`=22, `electric_to_resonance`=18, `micro_to_electric`=7

## 3. Welche Rückkopplung ist plausibel?

- `H_field_iono→H_field_reso`: r=+0.208 (weak) — Cavity-Hoehe/Leitfaehigkeit moduliert Frequenz und Q
