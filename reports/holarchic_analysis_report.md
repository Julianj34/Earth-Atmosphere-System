...
**Run:** 2026-09-17T22:56:20.637840+00:00  
**Snapshots:** 389  
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=219 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.254 | 219 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.193 | 218 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.275 | 389 | weak |
| H_field_gec→H_field_reso | field_feedback | +0.495 | 389 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 322/389 Snapshots (83%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=322, `none`=26, `micro_to_electric`=21, `electric_to_resonance`=20
