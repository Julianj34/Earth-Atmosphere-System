...
**Run:** 2026-08-09T20:57:48.790807+00:00  
**Snapshots:** 235  
...
| H_meso_conv | L2p5 | regional | 0.641 | proxy |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=65 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | +0.105 | 65 | negligible |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.294 | 64 | weak |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.526 | 235 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.522 | 235 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 206/235 Snapshots (88%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=206, `none`=22, `electric_to_resonance`=6, `micro_to_electric`=1
