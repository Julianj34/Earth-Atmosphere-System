...
**Run:** 2026-09-22T13:30:37.285183+00:00  
**Snapshots:** 407  
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=237 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.260 | 237 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.195 | 236 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.260 | 407 | weak |
| H_field_gec→H_field_reso | field_feedback | +0.496 | 407 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 337/407 Snapshots (83%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=337, `none`=27, `micro_to_electric`=22, `electric_to_resonance`=21
