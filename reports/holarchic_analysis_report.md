...
**Run:** 2026-08-13T09:38:31.231953+00:00  
**Snapshots:** 249  
...
| H_lithosphere | L1 | global | 0.378 | measured |
| H_macro_ocean | L2 | basin/global | 0.575 | measured |
| H_meso_conv | L2p5 | regional | 0.642 | proxy |
| H_micro_storm | L3 | local | 0.197 | measured |
| H_field_iono | L4 | global | 0.324 | measured |
| H_field_gec | L5 | global | 0.358 | measured |
| H_field_reso | L6 | global | 0.300 | proxy |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=79 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | +0.078 | 79 | negligible |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.210 | 78 | weak |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.492 | 249 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.526 | 249 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 214/249 Snapshots (86%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=214, `none`=22, `electric_to_resonance`=10, `micro_to_electric`=3
...
- `H_field_gec→H_field_reso`: r=+0.526 (moderate) — GEC ist die elektrische Architektur der Resonanz
