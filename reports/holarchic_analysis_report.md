...
**Run:** 2026-07-26T10:33:35.401864+00:00  
**Snapshots:** 178  
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy` bis echtes OLR zurueckkommt), aber die Historie (n=8) ist noch zu klein fuer eine belastbare Kopplungsschaetzung.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.536 | 8 | moderate_low_n |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.452 | 7 | moderate_low_n |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.531 | 178 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.501 | 178 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 152/178 Snapshots (85%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=152, `none`=21, `electric_to_resonance`=4, `micro_to_electric`=1

## 3. Welche Rückkopplung ist plausibel?

- `H_field_iono→H_field_reso`: r=+0.174 (negligible) — Cavity-Hoehe/Leitfaehigkeit moduliert Frequenz und Q
- `H_field_gec→H_field_reso`: r=+0.501 (moderate) — GEC ist die elektrische Architektur der Resonanz
