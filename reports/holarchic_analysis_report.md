...
**Run:** 2026-09-06T22:11:54.110886+00:00  
**Snapshots:** 345  
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=175 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.223 | 175 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.176 | 174 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.385 | 345 | weak |
| H_field_gec→H_field_reso | field_feedback | +0.512 | 345 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 295/345 Snapshots (86%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=295, `none`=22, `electric_to_resonance`=18, `micro_to_electric`=10
