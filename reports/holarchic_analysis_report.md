...
**Run:** 2026-08-12T04:20:25.736481+00:00  
**Snapshots:** 244  
...
| H_meso_conv | L2p5 | regional | 0.645 | proxy |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=74 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | +0.079 | 74 | negligible |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.238 | 73 | weak |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.513 | 244 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.525 | 244 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 212/244 Snapshots (87%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=212, `none`=22, `electric_to_resonance`=8, `micro_to_electric`=2
