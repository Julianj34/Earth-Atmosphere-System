...
**Run:** 2026-08-10T21:07:57.262282+00:00  
**Snapshots:** 239  
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=69 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | +0.099 | 69 | negligible |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.240 | 68 | weak |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.511 | 239 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.522 | 239 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 208/239 Snapshots (87%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=208, `none`=22, `electric_to_resonance`=7, `micro_to_electric`=2
