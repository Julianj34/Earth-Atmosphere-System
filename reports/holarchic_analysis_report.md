...
**Run:** 2026-07-26T21:16:05.373273+00:00  
**Snapshots:** 180  
...
| H_meso_conv | L2p5 | regional | 0.045 | proxy |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy` bis echtes OLR zurueckkommt), aber die Historie (n=10) ist noch zu klein fuer eine belastbare Kopplungsschaetzung.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.388 | 10 | weak_low_n |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.391 | 9 | weak_low_n |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.532 | 180 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.488 | 180 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 154/180 Snapshots (86%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=154, `none`=21, `electric_to_resonance`=4, `micro_to_electric`=1
