...
**Run:** 2026-08-11T21:10:48.329943+00:00  
**Snapshots:** 243  
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=73 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | +0.088 | 73 | negligible |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.239 | 72 | weak |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.512 | 243 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.524 | 243 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 211/243 Snapshots (87%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=211, `none`=22, `electric_to_resonance`=8, `micro_to_electric`=2
