...
**Run:** 2026-08-09T09:08:51.568730+00:00  
**Snapshots:** 233  
...
| H_meso_conv | L2p5 | regional | 0.636 | proxy |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=63 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | +0.107 | 63 | negligible |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.299 | 62 | weak |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.528 | 233 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.524 | 233 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 204/233 Snapshots (88%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=204, `none`=22, `electric_to_resonance`=6, `micro_to_electric`=1
