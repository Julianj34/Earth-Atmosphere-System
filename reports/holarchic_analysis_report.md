...
**Run:** 2026-08-24T09:09:32.742132+00:00  
**Snapshots:** 293  
...
| H_macro_ocean | L2 | basin/global | 0.576 | measured |
| H_meso_conv | L2p5 | regional | 0.651 | proxy |
| H_micro_storm | L3 | local | 0.195 | measured |
| H_field_iono | L4 | global | 0.325 | measured |
| H_field_gec | L5 | global | 0.350 | measured |
| H_field_reso | L6 | global | 0.296 | proxy |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=123 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.155 | 123 | negligible |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.142 | 122 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.489 | 293 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.516 | 293 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 258/293 Snapshots (88%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=258, `none`=22, `electric_to_resonance`=10, `micro_to_electric`=3
