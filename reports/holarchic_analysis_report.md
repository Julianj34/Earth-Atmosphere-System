...
**Run:** 2026-09-03T17:56:19.876678+00:00  
**Snapshots:** 332  
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=162 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.205 | 162 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | +0.152 | 162 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.449 | 332 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.513 | 332 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 291/332 Snapshots (88%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=291, `none`=22, `electric_to_resonance`=15, `micro_to_electric`=4
