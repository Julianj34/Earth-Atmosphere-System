...
**Run:** 2026-07-30T10:37:22.196828+00:00  
**Snapshots:** 194  
...
| H_meso_conv | L2p5 | regional | 0.427 | proxy |
| H_micro_storm | L3 | local | 0.197 | measured |
| H_field_iono | L4 | global | 0.326 | measured |
| H_field_gec | L5 | global | 0.370 | measured |
| H_field_reso | L6 | global | 0.306 | proxy |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=24 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | +0.282 | 24 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.250 | 23 | weak |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.533 | 194 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.509 | 194 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 166/194 Snapshots (86%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=166, `none`=22, `electric_to_resonance`=5, `micro_to_electric`=1
...
- `H_field_gec→H_field_reso`: r=+0.509 (moderate) — GEC ist die elektrische Architektur der Resonanz
