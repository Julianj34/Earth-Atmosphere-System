...
**Run:** 2026-09-21T15:12:23.603332+00:00  
**Snapshots:** 403  
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=233 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.261 | 233 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.201 | 232 | weak |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.266 | 403 | weak |
| H_field_gec→H_field_reso | field_feedback | +0.497 | 403 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 334/403 Snapshots (83%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=334, `none`=27, `micro_to_electric`=21, `electric_to_resonance`=21
