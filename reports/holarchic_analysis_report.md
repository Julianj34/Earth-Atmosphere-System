...
**Run:** 2026-07-27T16:38:20.331988+00:00  
**Snapshots:** 183  
...
| H_lithosphere | L1 | global | 0.381 | measured |
| H_macro_ocean | L2 | basin/global | 0.571 | measured |
| H_meso_conv | L2p5 | regional | 0.188 | proxy |
| H_micro_storm | L3 | local | 0.198 | measured |
| H_field_iono | L4 | global | 0.326 | measured |
| H_field_gec | L5 | global | 0.373 | measured |
| H_field_reso | L6 | global | 0.308 | proxy |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy` bis echtes OLR zurueckkommt), aber die Historie (n=13) ist noch zu klein fuer eine belastbare Kopplungsschaetzung.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.175 | 13 | negligible_low_n |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | +0.152 | 13 | negligible_low_n |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.533 | 183 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.487 | 183 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 156/183 Snapshots (85%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=156, `none`=21, `electric_to_resonance`=5, `micro_to_electric`=1

## 3. Welche Rückkopplung ist plausibel?

- `H_field_iono→H_field_reso`: r=+0.204 (weak) — Cavity-Hoehe/Leitfaehigkeit moduliert Frequenz und Q
- `H_field_gec→H_field_reso`: r=+0.487 (moderate) — GEC ist die elektrische Architektur der Resonanz
