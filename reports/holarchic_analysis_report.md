...
**Run:** 2026-09-21T19:35:54.348519+00:00  
**Snapshots:** 404  
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=234 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.260 | 234 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.197 | 233 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.264 | 404 | weak |
| H_field_gec→H_field_reso | field_feedback | +0.495 | 404 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 335/404 Snapshots (83%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=335, `none`=27, `micro_to_electric`=21, `electric_to_resonance`=21

## 3. Welche Rückkopplung ist plausibel?

- `H_field_iono→H_field_reso`: r=+0.247 (weak) — Cavity-Hoehe/Leitfaehigkeit moduliert Frequenz und Q
