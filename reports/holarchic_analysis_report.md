...
**Run:** 2026-09-22T23:03:44.522886+00:00  
**Snapshots:** 409  
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=239 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.262 | 239 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.202 | 238 | weak |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.259 | 409 | weak |
| H_field_gec→H_field_reso | field_feedback | +0.495 | 409 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 339/409 Snapshots (83%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=339, `none`=27, `micro_to_electric`=22, `electric_to_resonance`=21
