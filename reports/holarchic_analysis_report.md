...
**Run:** 2026-08-11T15:20:32.601301+00:00  
**Snapshots:** 242  
...
| H_field_gec | L5 | global | 0.360 | measured |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=72 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | +0.088 | 72 | negligible |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.240 | 71 | weak |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.512 | 242 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.524 | 242 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 210/242 Snapshots (87%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=210, `none`=22, `electric_to_resonance`=8, `micro_to_electric`=2

## 3. Welche Rückkopplung ist plausibel?

- `H_field_iono→H_field_reso`: r=+0.198 (negligible) — Cavity-Hoehe/Leitfaehigkeit moduliert Frequenz und Q
