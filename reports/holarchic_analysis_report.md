...
**Run:** 2026-09-25T23:23:02.439675+00:00  
**Snapshots:** 421  
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=251 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.277 | 251 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.192 | 250 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.242 | 421 | weak |
| H_field_gec→H_field_reso | field_feedback | +0.490 | 421 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 347/421 Snapshots (82%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=347, `none`=27, `micro_to_electric`=25, `electric_to_resonance`=22
