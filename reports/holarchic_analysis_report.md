...
**Run:** 2026-08-04T05:29:43.246298+00:00  
**Snapshots:** 213  
...
| H_meso_conv | L2p5 | regional | 0.561 | proxy |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=43 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | +0.075 | 43 | negligible |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.242 | 42 | weak |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.524 | 213 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.515 | 213 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 185/213 Snapshots (87%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=185, `none`=22, `electric_to_resonance`=5, `micro_to_electric`=1

## 3. Welche Rückkopplung ist plausibel?

- `H_field_iono→H_field_reso`: r=+0.164 (negligible) — Cavity-Hoehe/Leitfaehigkeit moduliert Frequenz und Q
- `H_field_gec→H_field_reso`: r=+0.515 (moderate) — GEC ist die elektrische Architektur der Resonanz
