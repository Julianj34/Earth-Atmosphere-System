...
**Run:** 2026-08-01T10:15:07.043175+00:00  
**Snapshots:** 202  
...
| H_meso_conv | L2p5 | regional | 0.487 | proxy |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=32 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | +0.213 | 32 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.206 | 31 | weak |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.527 | 202 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.519 | 202 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 174/202 Snapshots (86%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=174, `none`=22, `electric_to_resonance`=5, `micro_to_electric`=1

## 3. Welche Rückkopplung ist plausibel?

- `H_field_iono→H_field_reso`: r=+0.185 (negligible) — Cavity-Hoehe/Leitfaehigkeit moduliert Frequenz und Q
- `H_field_gec→H_field_reso`: r=+0.519 (moderate) — GEC ist die elektrische Architektur der Resonanz
