...
**Run:** 2026-07-25T11:55:57.237575+00:00  
**Snapshots:** 175  
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy` bis echtes OLR zurueckkommt), aber die Historie (n=2) ist noch zu klein fuer eine belastbare Kopplungsschaetzung.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | — | 2 | insufficient_n |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | — | 2 | insufficient_n |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.514 | 175 | moderate |
| H_field_gec→H_field_reso | field_feedback | +0.484 | 175 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 151/175 Snapshots (86%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=151, `none`=19, `electric_to_resonance`=4, `micro_to_electric`=1
